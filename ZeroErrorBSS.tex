...
\section{Formal Description of the Remote State Estimation Setup}	\label{sec:FormalEstimationSetup}
	...
	\begin{align*}	 \rs_{t+1}    	= \mA \rs_t + \rz_t + \rv_1, 
	\end{align*}
	...
	Last but not least, at the receiving end of the channel, the plant's state sequence is estimated and a control input is calculated,
	\begin{align*}	\hat{\rs}_t  	&= \De \big(t, (\ry_{t'})_{t'=1}^{t}\big)  \\
									&= \De\big(t,\ry_1,\ry_2,\ldots,\ry_t\big), \\
					\rv_t  			&= \Dc \big(t, (\ry_{t'})_{t'=1}^{t}\big)  \\
									&= \Dc\big(t,\ry_1,\ry_2,\ldots,\ry_t\big).
	\end{align*}
	The generated control input is fed back to the dynamic system, thereby closing the control loop.
	...
	relevant of which are the \emph{Shannon capacity} \sdummy, the \emph{zero-error capacity} \sdummy and the \emph{feedback zero-error capacity} \sdummy, 
	...
	which capacity has to be employed as a measure of channel quality.
	...
	It remains to provide a formal objective regarding the estimation and control of the system's state sequence. The following variants have been considered
	in \cite{MS07, MS07SC}:
	
	1. - \emph{State Estimation for Disturbed Plants.} The strong objective in remote state estimation requires almost sure observability. By a suitable choice of \(\E\) and \(\D\), the error between \((s(t))_{t\in\NN}\)
