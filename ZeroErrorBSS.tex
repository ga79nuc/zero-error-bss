...
\begin{abstract}
	...
	memoryless channel \emph{(DMC)}, does there exist a pair of encoder and decoder that allows for the remote tracking of the system's 
...
\section{Formal Description of the Remote State Estimation Setup}	\label{sec:FormalEstimationSetup}
	...
	For the exact assumptions, we refer to...
...
\section{Preliminaries from the Theory of Zero-Error Coding}	\label{sec:PreliminariesZeroError}
	\noindent As indicated in Section \label{sec:FormalEstimationSetup}, we consider finite alphabets \(\X\) and \(\Y\) as well as conditional probability mass functions on \(\Y\times\X\).
	...
	The theory of zero-error coding, as introduced in \cite{S56}, is a sub-discipline of general channel coding. In essence, it considers the problem of finding the supremum 
	...
	Given a DMC \((\X,\Y,W)\), one of the results derived in \cite{S56} yields a characterization of \(C_0(W)\) in graph-theoretic terms.
	...
	\begin{Theorem}	[\cite{S56}]\label{thm:ZE_Capacity}
...
\section{Deciding the Solvability of the Remote State Estimation Problem on Blum-Shub-Smale Machines}	\label{sec:DecidingRemoteStateEstimationOnBSS}
	...
	\begin{Theorem}	Let \(\X,\Y\) be finite alphabets and \(\mA \in \RR^{n\times n}\) a characterization of an unstable, time-invariant, 
