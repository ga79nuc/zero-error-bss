...
\section{Preliminaries from the Theory of Turing Machines and Effective Analysis}\label{sec:introTuring}
	more conservative approach. unless positive proof is provided by the algorithm
...
\section{Summary and Additional Remarks}	\label{sec:Conclusion}
	...
	On the one hand, we have introduced a class of binary channel classification problems  \(\M_j \subset \Wc : j \in \{0,1\}\) such that the case of \( j = 0\) is 
	...
	Furthermore, we have shown that the classification problem \((\S_\star(\mA),\U_\star(\mA)) = (\M_0,\M_1) : \star \in \{\text{'\(0\)'},\text{'\(0,\mathrm{f}\)'}\}\), 
	which characterizes the solvability of the remote state estimation and stabilization problem, belongs to this class. 
	
	On the other hand, we have proven the existence of a Blum-Shub-Smale algorithm that computes the zero-error and/or feedback zero-error capacity
	\(C_\star : \W \rightarrow \RR_{\hspace{1pt}0}^{+}, \star \in \{\text{'\(0\)'},\text{'\(0,\mathrm{f}\)'}\}\) for channels \(W\in\W\). In turn, 
	we have used this result to provide a Blum-Shub-Smale algorithm that decides the classification problem
	\((\S_\star(\mA),\U_\star(\mA)) = (\M_0,\M_1) : \star \in \{\text{'\(0\)'},\text{'\(0,\mathrm{f}\)'}\}\).
	Both algorithms rely essentially on the ability of Blum-Shub-Smale machines to process exact real numbers.   
	
	{\color{red} add: bezug zur introduction, applied coparison, differences of models and so on} 
	...
	{\color{violet}% 
	%Remote state estimation and stabilization via discrete, memoryless channels belongs to a class of engineering problems that arereferred to as \emph{co-design} tasks. 
	...
	%On the one hand, the amount of information that needs to be transmitted is in all cases quantified by the number \(\eta(\mA)\). On the other hand, 
	%the we have seen that the specific control objective determines whether \(C\), \(C_0\) or \(C_{\text{'\(0,\mathrm{f}\)'}}\) is the suitable measure of channel quality.
	%-> hence channel design cannot be done soely on amount of info, but requires knowledge about ctrl objective; ctrl and info part dont decompose,
	%hence co design problem.
	...
	-> in the boradest sense: also co design of hardware platform since bss stronger than turing.
	 is an explicit example of control and communications co-design. 
	
	-> co desing problem, thing does not decompose into individual parts ....
	
	The criterion imposed on the control performance
