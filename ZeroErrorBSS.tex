...
\section{Introduction}	\label{sec:Introduction}
	...
	\begin{figure}\linespread{1}
		...
		\begin{tikzpicture}[scale = 0.7, every node/.style={scale=0.7}]
			...
			%   %   %   %   %   %   %   %   %   %   %
			%   Decoder
				\fill[fill=black!15] (-6.5,-2) rectangle (-4.5,-1);
...
\section{Turing Computability and Channels (Working Title)}
	...
	\end{Lemma}\begin{proof}
					...
					\begin{align}	g(n,m) := 	\begin{cases}	1 &\text{if~} n \in \big\{f_\N(l) : 1 \leq l \leq m\big\}, \\ 	
																0 &\text{otherwise}.
												\end{cases}
					\end{align}
					Then, \(g: \NN^2 \rightarrow \NN\) is a total recursive function that satisfies the following:
					\begin{itemize}	\item 	for all \(n,m,l\in\NN\) that satisfy \(m\leq l\), we have \linebreak \(g(n,m) = 1 \Rightarrow g(n,l) = 1\);
									\item	for all \(n\in\NN\), there exists \(m\in\NN\) such that \(g(n,m) = 1\) is satisfied if and only if 
											\(n\in \N\) holds true.
					\end{itemize}  
	\end{proof}
	\begin{Lemma}	Let \(\X,\Y\) be finite Alphabets and \(\M_0, \M_1\) disjoint subsets of \(\Wc(\X,\Y)\) that satisfy the following:
