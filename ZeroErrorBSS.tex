...
\section{Formal Description of the Remote State Estimation Setup}	\label{sec:FormalEstimationSetup}
	...
	In the following, we use the greek letter '\(\Delta\)' as a placeholder symbol. In light of the results established in \cite{MS07, MS07SC}, we define 
	the sets 
	\begin{align}	\S_\Delta(\mA) :&= \big\{ W \in \W(\X,\Y) : C_\Delta > \eta(\mA)\big\}, \\
					\U_\Delta(\mA) :&= \big\{ W \in \W(\X,\Y) : C_\Delta < \eta(\mA)\big\},
	\end{align}
	for \(\Delta \in \{\text{'Sh'},\text{'\(0\)'},\text{'Fb'}\}\) and \(\mA\in\RR^{n\times n}\).
